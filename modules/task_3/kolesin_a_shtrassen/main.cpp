...
TEST(Count_Words, Test_SeqVsSimple) {
    ...
    if (rank == 0) {
        int n = 8;
        std::vector<double> m1 = getRandomMatrix(n);
        std::vector<double> m2 = getRandomMatrix(n);
        std::vector<double> seq = m1;
        std::vector<double> simp = m1;
        Matrix M1(&m1[0], n);
        Matrix M2(&m2[0], n);
        Matrix Seq(&seq[0], n);
...
TEST(Count_Words, Test_ParallVsSeq_16) {
    ...
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    int n = 16;
    std::vector<double> m1(n * n);
    std::vector<double> m2(n * n);
    if (rank == 0) {
