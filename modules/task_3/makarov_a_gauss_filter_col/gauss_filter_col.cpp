...
std::vector<unsigned __int16> gaussFilterParallel(
    ...
    if (rank == 0) {
        ...
        for (int i = 2; i < proc_count; i++) {
            ...
            MPI_Send(t_image.data() + start_pos, col_count, MPI_SHORT, i, 0,
                     MPI_COMM_WORLD);
        }
        MPI_Status status;
        ...
        for (int i = 2; i < proc_count; i++) {
            ...
            MPI_Recv(tmp.data(), delta * h, MPI_SHORT, i, MPI_ANY_TAG,
                     MPI_COMM_WORLD, &status);
        ...
        if (rank == 1) {
            ...
            MPI_Recv(local_image.data(), first_count, MPI_SHORT, 0,
                     MPI_ANY_TAG, MPI_COMM_WORLD, &status);
            local_result = transpose(gaussFilter(local_image, h,
                                     remain + delta, sigma, radius), h,
                                     remain + delta);
            MPI_Send(local_result.data(), (remain + delta) * h, MPI_SHORT, 0,
                     0, MPI_COMM_WORLD);
        } else {
            local_image.resize(col_count);
            MPI_Recv(local_image.data(), col_count, MPI_SHORT, 0, MPI_ANY_TAG,
                                                     MPI_COMM_WORLD, &status);
            local_result = transpose(
                            gaussFilter(local_image, h, delta, sigma, radius),
                            h, delta);
            MPI_Send(local_result.data(), delta * h, MPI_SHORT, 0, 0,
                                                       MPI_COMM_WORLD);
        }
        return local_result;
    }
